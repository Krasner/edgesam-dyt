...
def main(args, config):
    ...
    if not config.DISTILL.ENCODER_ONLY:
        _m = build_sam_from_config(config, None, True, True, use_original_mask_decoder=True)
        if not args.only_cpu:
            _m.cuda()

        prompt_encoder_t = copy.deepcopy(_m.prompt_encoder)
        ...
        msg = prompt_encoder_t.load_state_dict(new_state_dict, strict=False)
        logger.warning(msg)
        logger.info(f"=> loaded successfully 'prompt_encoder_teacher'")

        mask_decoder_t = copy.deepcopy(_m.mask_decoder)
        ...
        # breakpoint()
        new_state_dict = {}
        for k, v in mask_decoder_weights.items():
            _k = k.replace("mask_decoder.","")
            new_state_dict[_k] = v

        # msg = mask_decoder_t.load_state_dict(mask_decoder_weights, strict=False)
        msg = mask_decoder_t.load_state_dict(new_state_dict, strict=False)
        logger.warning(msg)
        ...
        if not config.DISTILL.ENCODER_ONLY:
            ...
            if config.DISTILL.INIT_FROM_TEACHER:
                ...
                for k, v in mask_decoder_weights.items():
                    _k = k.replace("mask_decoder.","")
                    new_state_dict[_k] = v
                msg = model_without_ddp.mask_decoder.load_state_dict(new_state_dict, strict=False)
            ...
            if config.DISTILL.FREEZE_IMAGE_ENCODER:
                ...
                if config.DISTILL.IMAGE_ENCODER_DYT:
                    for param in model_without_ddp.image_encoder.parameters():
                        param.requires_grad = False
                    for name, param in model_without_ddp.image_encoder.named_parameters():
                        if (
                            ('fuse_stage2' in name)
                            or ('fuse_stage3' in name)
                            or ('neck' in name)
                        ):
            ...
            if config.DISTILL.FREEZE_MASK_DECODER:
                ...
                if config.DISTILL.ONLY_TUNE_MASK_DECODER_DYT:
                    # output_upscaling
                    # transformer.norm layers (5 total, norm - 1,2,3,4, final)

                    # first freeze all
                    for param in model_without_ddp.mask_decoder.parameters():
                        param.requires_grad = False

                    # unlock only dynamic tanh layers
                    for name, param in model_without_ddp.mask_decoder.named_parameters():
                        ...
                        if 'transformer' in name and 'norm' in name:
                            print(f"{name} set to trainable")
                            param.requires_grad = True
                        elif 'output_upscaling.1' in name:
                            param.requires_grad = True
                            print(f"{name} set to trainable")
            ...
            if config.DISTILL.IMAGE_ENCODER_DYT:
                ...
                for name, param in model_without_ddp.named_parameters():
                    if (
                        ...
                        print(f"{name} set to trainable")
            """
            ['fuse_stage2.weight', 'fuse_stage3.op_list.0.weight', 'neck.0.weight', 'neck.1.alpha', 'neck.1.weight', 'neck.1.bias', 'neck.2.weight', 'neck.3.alpha', 'neck.3.weight', 'neck.3.bias']
...
def train_one_epoch_distill_using_saved_embeddings(args, config, model, data_loader, optimizer, epoch,
    ...
    for idx, ((samples, annos), (saved_embeddings, seeds)) in enumerate(data_loader):
        ...
        with torch.cuda.amp.autocast(enabled=config.AMP_ENABLE):# , device_type="cuda"):
            if config.DISTILL.ENCODER_ONLY:
